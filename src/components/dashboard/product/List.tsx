...
const ProductDataTable = () => {
  ...
  const { data: products, isLoading, error } = useItemsWithPagination<ProductResponse, PaginationResponse<ProductResponse>>('products', { page, perPage, search });
    ...
    <div className="w-full p-4 rounded-lg shadow-sm">
      <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="right-3">
          <Link href="/dashboard/products/create" className="p-2 bg-blue-600 text-white cursor-pointer rounded-md">Create</Link>
        </div>
      </div>
      <div className="relative flex items-center gap-1">
        ...
        >Search</button>
        <button
          className="bg-gray-200 px-4 py-2 cursor-pointer rounded-md"
          onClick={() => resetData()}
        >Reset</button>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full border-collapse">
          ...
          <tbody>
            {
              isLoading && (
                <tr><td colSpan={6} className="p4 text-center">Loading...</td></tr>
              )
            }
            {
              error && (
                <tr><td colSpan={6} className="p4 text-center">{error.message}</td></tr>
              )
            }
              ...
              >
                ...
                <td className="p-4">
                  <div className="w-full flex items-center justify-center gap-2">
                    <Link href={`/dashboard/products/${item.id}`}><Pencil /></Link>
                    <button
